...
ipath_goes = 'C:/Users/rodney.viereck/Documents/Python/EUV_2/stan_bands/'
ifile_goes = "GOES_Stan_Bands.dat"
...
#Load GOES Stand Bands

goes_sb_df = pd.read_csv(ipath_goes+ifile_goes)
goes_sb_df['Datetime'] = pd.to_datetime(goes_sb_df['Datetime'])
goes_sb_df.set_index('Datetime', inplace = True)
goes_columns = goes_sb_df.columns
...
if plot_spec:
    ...
    goes_spec = goes_sb_df.loc[(goes_sb_df.index >= str_date)
                         & (goes_sb_df.index < end_date)].mean(axis=0)
    ...
    for i1 in range (37):
        if (i1 >= 11) & (i1 <= 18):
            t1 = (goes_columns[i1].split("_")[0])
            plt_spec_wl[2*i1] = float(t1.split("-")[0])
            plt_spec_wl[2*i1+1] = float(t1.split("-")[1])
            print(i1,t1,plt_spec_wl[2*i1])
        else:
            plt_spec_wl[2*i1] = float(goes_columns[i1].split("-")[0])
            plt_spec_wl[2*i1+1] = float(goes_columns[i1].split("-")[1])
            
        plt_goes_spec[2*i1] = goes_spec.iloc[i1] * ymult_goes
        plt_goes_spec[2*i1+1] = goes_spec.iloc[i1]* ymult_goes
    ...
    ax1.plot(plt_spec_wl,plt_goes_spec,color='red', label = "SEE Spectrum")
